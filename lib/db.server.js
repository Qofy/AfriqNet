...
const staticMusicVideos = [
  {
    ...
    duration: '3:45',
    views: '15.2M',
    stream: '/music/oil-on-my-head.mp4'
  },
  {
    id: 'mv2',
    title: 'Stir Up',
    artist: 'Sarkodie ft. Bob Marley',
    type: 'Music Video',
    poster: '/images/music/stir-up.jpg',
    backdrop: '/images/music/s-u-link2.jpg',
    rating: 9.5,
  ...
  },
  {
    ...
    duration: '3:28',
    views: '18.5M',
    stream: '/music/ten-toes(king-promise).mp4'
  },
  {
    id: 'mv4',
    title: 'Therapy',
    ...
    duration: '4:35',
    views: '25.1M',
    stream: '/music/therapy(stone-bwoy).mp4'
  },
  {
    id: 'mv5',
  ...
  },
  {
    ...
    duration: '3:15',
    views: '20.2M',
    stream: '/trailer/chief-of-war.mp4'
  },
  {
    id: 'mv9',
    title: 'Love & Light',
    ...
    duration: '4:05',
    views: '19.8M',
    stream: '/trailer/Under-the-stars.mp4'
  },
  {
    id: 'mv10',
    title: 'Heritage',
...
export async function updateUserProfileImage(userId, url) {
  await firestoreDb.collection('users').doc(userId).update({ profile_image: url });
  return getUserById(userId);
}

export async function updateUserPassword(userId, passwordHash) {
  await firestoreDb.collection('users').doc(userId).update({ password: passwordHash });
}
...
export async function createSession({ id, userId, expiresAt }) {
  const now = Date.now();
  await firestoreDb.collection('sessions').doc(id).set({
    user_id: userId,
...
export async function getSession(id) {
  const doc = await firestoreDb.collection('sessions').doc(id).get();
  if (!doc.exists) return null;
  const data = doc.data();
  return {
    id: doc.id,
...
export async function upsertWatchProgress({ userId, contentId, position, duration }) {
  ...
  await firestoreDb.collection('watch_progress').doc(docKey).set(
    {
      user_id: userId,
...
export async function getWatchProgress(userId, contentId) {
  const docKey = `${userId}_${contentId}`;
  const doc = await firestoreDb.collection('watch_progress').doc(docKey).get();
  if (!doc.exists) return null;
  const data = doc.data();
  return {
    id: doc.id,
...
export async function deleteWatchProgressForUser(userId) {
  const snap = await firestoreDb.collection('watch_progress').where('user_id', '==', userId).get();
  const batch = firestoreDb.batch();
  snap.docs.forEach(doc => batch.delete(doc.ref));
  if (snap.docs.length > 0) await batch.commit();
}
