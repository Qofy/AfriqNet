...
try {
  ...
  if (movieCount === 0) {
    ...
    const movieInsert = db.prepare(`
      INSERT INTO movies (id, title, type, poster, backdrop, rating, release_date, overview, genre_ids, runtime, tagline, video_stram)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);

    const movies = [
      ['m1', "All The Devils Are There", 'Movie', 'https://images.unsplash.com/photo-1536440136628-849c177e76a1?w=500&q=80', 'https://images.unsplash.com/photo-1536440136628-849c177e76a1?w=1920&q=80', 8.5, '2024-01-15', "In a world where quantum computing has unlocked parallel dimensions, a brilliant physicist must prevent a catastrophic collision between realities.", '878,53,28', 142, 'Reality is just the beginning', ""],
...
export function getAllMovies() {
  const stmt = db.prepare(`
    SELECT id, title, type, poster, backdrop, rating, release_date, overview, 
           genre_ids, runtime, tagline, video_stram FROM movies ORDER BY rating DESC
  `);
...
export function getMovieById(id) {
  const stmt = db.prepare(`
    SELECT id, title, type, poster, backdrop, rating, release_date, overview, 
           genre_ids, runtime, tagline, video_stram FROM movies WHERE id = ?
  `);
